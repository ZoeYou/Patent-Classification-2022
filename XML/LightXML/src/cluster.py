...
if __name__ == '__main__':
    ...
    mlb = build_tree_by_level(f'./data/{dataset}/train_v1.txt', 
                                f'./data/{dataset}/train_labels.txt',
                                1e-4, 100, [], f'./data/{dataset}/label_group'+args.id)
    groups = np.load(f'./data/{dataset}/label_group{args.id}-last.npy', allow_pickle=True)
    new_group = []
    for group in groups:
