...
class Model(object):
    ...
    def __init__(self, network, model_path, gradient_clip_value=5.0, device_ids=None, **kwargs):
        ...
        # self.model = nn.DataParallel(network(**kwargs).cuda(), device_ids=device_ids) #change DataParallel to parallel.DistributedDataParallel
        self.model = network(**kwargs).cpu()
        ...
        if self.gradient_clip_value is not None:
            ...
            if total_norm > max_norm * self.gradient_clip_value:
                logger.warn(F'Clipping gradients with total norm  '
                            F'and max norm ')# round(total_norm, 5) round(max_norm, 5)
...
class XMLModel(Model):
    ...
    def __init__(self, labels_num, hidden_size, device_ids=None, attn_device_ids=None,
        ...
        self.model = nn.ModuleDict({'Network': self.network, 'AttentionWeights': self.attn_weights})
        self.state['best'] = {}
    ...
    def predict_step(self, data_x: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], k):
        ...
        with torch.no_grad():
            ...
            scores, labels = torch.topk(scores * group_scores, k)
