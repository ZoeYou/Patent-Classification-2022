...
def load_word2vec(params):
    ...
    inputs:
    model_type      # GoogleNews / glove / fasttext
    vocabulary_inv  # dict {str:int}
    num_features    # Word vector dimensionality
    """
    ...
    #    print('Loading existing Word2Vec model (Glove.6B.%dd)' % (params.num_features))
    ...
    elif params.model_type == 'fasttext':
        model_name = join(model_dir, 'wiki.fr.vec')
        print(model_name)
        assert(exists(model_name))
        print('Loading existing Word2Vec model wiki.fr.vec')
        ...
        for line in open(model_name, 'r'):
            cnt += 1
            if cnt == 1:    # remove the first row
                continue
    
            tmp = line.strip().split(' ')
            word, vec = tmp[0], [float(e) for e in tmp[1:]]
        ...
        print('Size of embedding model: ', len(embedding_model))
