...
class FastAttentionXML(object):
    ...
    def train_level(self, level, train_x, train_y, valid_x, valid_y):
        ...
        train_y = csr_matrix(train_y)
        valid_y = csr_matrix(valid_y)
        if level == 0:
            ...
            train_loader = DataLoader(MultiLabelDataset(train_x, train_y),
                                      model_cnf['train'][level]['batch_size'], num_workers=4, shuffle=True)#change num_workers from 4 to 0
            valid_loader = DataLoader(MultiLabelDataset(valid_x, valid_y, training=False),
                                      model_cnf['valid']['batch_size'], num_workers=4)#change num_workers from 4 to 0
            ...
            logger.info(F'group_candidates.shape: {group_candidates.shape}')
