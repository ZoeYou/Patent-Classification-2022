...
class PatentMultiLabelProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        """See base class."""
        tf.compat.v1.logging.info('get_train_examples: %s' % FLAGS.train_file)
        return self._create_examples(
                self._read_tsv(os.path.join(data_dir, FLAGS.train_file)), 'train')

    def get_dev_examples(self, data_dir):
        """See base class."""
        tf.compat.v1.logging.info('get_dev_examples: %s' % FLAGS.dev_file)
        return self._create_examples(
                self._read_tsv(os.path.join(data_dir, FLAGS.dev_file)), 'dev')

    def get_test_examples(self, data_dir):
        """See base class."""
        tf.compat.v1.logging.info('get_test_examples: %s' % FLAGS.test_file)
        return self._create_examples(
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if i % 5000 == 0:
                tf.compat.v1.logging.info('_create_examples: %s' % i)
        ...
        tf.compat.v1.logging.info('_create_examples(): done')
        return examples
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  if FLAGS.task_name == 'PMLP':
    name_to_features = {
        ...
        "label_ids": tf.io.FixedLenFeature([FLAGS.num_classes], tf.int64), 
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("loss"):
    ...
    if FLAGS.task_name == 'PMLP':
      ...
      tf.compat.v1.logging.info('multi_hot_prediction: %s' % multi_hot_prediction)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    tf.compat.v1.logging.info('num_labels = %s' % num_labels)
...
def check_parameters(processors):  
  ...
  if tf.io.gfile.Exists(FLAGS.output_dir):
    tf.compat.v1.logging.info('Existed: %s ' % FLAGS.output_dir)
  else:
...
def show_parameters_pmlp(label_list):    
  tf.compat.v1.logging.info('labels: %s' % label_list)
  tf.compat.v1.logging.info('FLAGS.num_classes = %s' % FLAGS.num_classes)
  if len(label_list) == 0:
    tf.compat.v1.logging.info('Empty label list')
    pdb.set_trace()
  tf.compat.v1.logging.info('multi_hot_threshold: %s' % FLAGS.multi_hot_threshold)
...
def configure_tpu():
  ...
  tf.compat.v1.logging.info('RunConfig: done')
  
  return run_config


def save_pred_result(label_list, all_y_lables, all_y_multi_hot_prediction, all_y_probabilities):
  result = []
  ...
  tf.compat.v1.logging.info('saved: %s' % fn)           
...
def show_predict_result_pmlp(label_list, result, num_actual_predict_examples):
  ...
  all_y_probabilities = []
  tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.WARN)  # avoid INFO message 
  tf.compat.v1.logging.info("***** Predict results *****")
  try:
    for (i, prediction) in enumerate(result):
        if i % 1000 == 0:
            tf.compat.v1.logging.warning('i = %s / %s' % (i, num_actual_predict_examples))
    ...
    tf.compat.v1.logging.warning('%s' % str(e))
    tf.compat.v1.logging.warning('(my own log) Assuming this is caused by padded examples saved in tf records and should be avoided in non-TPU mode')
  tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.INFO)
    
  if FLAGS.save_pred_result == True:
    save_pred_result(label_list, all_y_lables, all_y_multi_hot_prediction, all_y_probabilities)


def run_predict(estimator, predict_examples, label_list, tokenizer, task_name):
  tf.compat.v1.logging.info("***** Running prediction*****")
  tf.compat.v1.logging.info("  Batch size = %d", FLAGS.predict_batch_size)
  tf.compat.v1.logging.info("  file_based_flag = %s" % FLAGS.file_based_flag)
  ...
  if FLAGS.file_based_flag == True:
    ...
    if FLAGS.reuse_tf_record == True and tf.io.gfile.Exists(predict_file):
        tf.compat.v1.logging.info('Reuse: %s' % predict_file)
    else:
        file_based_convert_examples_to_features(predict_examples, label_list, FLAGS.max_seq_length, tokenizer, predict_file)
    
    tf.compat.v1.logging.info("  Num examples = %d (%d actual, %d padding)",len(predict_examples), num_actual_predict_examples,len(predict_examples) - num_actual_predict_examples)
  ...
  if task_name == 'pmlp':
    ...
    tf.compat.v1.logging.info('not implemented yet')
    pdb.set_trace()

  tf.compat.v1.logging.info('predict: done')
###############################################################################################
